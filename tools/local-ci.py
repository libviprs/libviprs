...
def main():
    ...
    print(f"==> building {IMAGE} (cached after the first run)")
    subprocess.run(
        ["docker", "build", "-q", "-f", f"{REPO}/tools/Dockerfile.ci", "-t", IMAGE, f"{REPO}/tools"],
    ...
    for j in plan:
        ...
        proc = subprocess.Popen(
            ["docker", "run", "--rm"] + mounts + ["-w", "/src/libviprs", IMAGE,
             "bash", "-c", container_script(j)],
