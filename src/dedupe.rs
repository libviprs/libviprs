...
//! the canonical order the crate-internal MapReduce hot-tile cache orders by
//! too. The layout shape is otherwise identical; only *which* occurrence keeps
//! the full bytes changes, and it changes to a value fixed by the input alone.
//! The reassignment pass itself is `FsSink`'s private
//! `canonicalize_dedupe_layout`, run once from its `finish()`.
//!
